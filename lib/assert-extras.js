...
assert.isWritable = function (object, property, message) {
    ...
    var newValue = original + " overwritten";
...
assert.isNotWritable = function (object, property, message) {
    ...
    if (object[property] == newValue) {
        object[property] = original;
        assert.fail(object, property, message, "isNotWritable", assert.isWritable);
    }
}
...
assert.isConfigurable = function (object, property, message) {
    ...
    if (object.hasOwnProperty(property)) {
        assert.fail(object, property, message, "isConfigurable", assert.isWritable);
    }
...
assert.isNotConfigurable = function (object, property, message) {
    ...
    if (!object.hasOwnProperty(property)) {
        object[property] = original;
        assert.fail(object, property, message, "isNotConfigurable", assert.isWritable);
    }
}

assert.isEnumerable = function (object, property, message) {
    if (Object.keys(object).indexOf(property) < 0) {
        assert.fail(object, property, message, "isEnumerable", assert.isWritable);
    }
}

assert.isNotEnumerable = function (object, property, message) {
    if (Object.keys(object).indexOf(property) >= 0) {
        assert.fail(object, property, message, "isNotEnumerable", assert.isWritable);
    }
}
